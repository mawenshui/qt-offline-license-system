...
- 创建、打开、锁定密钥库。
- 生成根密钥、签发密钥、产品加密密钥。
- 展示公钥指纹、解锁状态和 10 分钟自动锁定倒计时；最后 60 秒告警。
- 锁定时禁用 Runtime 配置导出、单个签发和批量签发。
- 复制加密 `.qtkv` 备份，拒绝覆盖并核对 SHA-256。
- 使用正式密钥库打开流程验证备份可恢复，并核对产品、密钥 ID 和根公钥。
- 验证本地审计日志哈希链。

首次启动显示四步向导；危险操作必须二次确认，但不得显示或复制明文私钥。
...
永久授权被选中时，日期控件必须禁用并清空，避免残留时间进入载荷。当前载荷版本 1 只允许目标 Runtime SDK 1.x；不兼容时禁止签发。

### 15.3 批量签发页

- 导入 UTF-8 CSV 或 JSON。
- 选择输出目录和目标 Runtime SDK 系列。
- 批次前显示输入、输出和兼容性确认摘要。
- 全量预检后写入临时目录，全部成功才原子发布。
- 输出逐文件 SHA-256 和签名的 `batch-manifest.json`。
...
按月写入当前用户本地应用数据目录下的 `audit/audit-YYYY-MM.jsonl`。每条记录包含 `previous_hash` 和 `record_hash`，追加前验证现有链；签发器提供只读验证操作。

记录：

- UTC 时间、操作员、操作类型和成功/失败结果。
- 产品、业务主体、输出路径和文件 SHA-256。
- 可安全公开的结果说明。
...
| `AuditLogger` | 结构化 JSONL 审计、哈希链追加和篡改验证 |
| `RuntimeCompatibility` | 授权格式与目标 Runtime SDK 系列预检 |
...
- 优先级：High
- 要求：签发端关键操作记录哈希链审计；日志不得包含口令、私钥、产品解密密钥、原始硬件值或完整载荷。
- 验收：连续记录可验证；修改任意既有记录后验证失败；敏感字段扫描为零。

### SR-010 签发兼容与状态门控

- 优先级：High
- 要求：密钥库锁定时禁止签发和导出；签发前校验目标 Runtime 系列与授权格式兼容。
- 验收：GUI 状态测试确认主操作禁用；不兼容 Runtime 由核心检查拒绝。
...
- 锁定状态下导出、单个签发和批量签发禁用。
- 目标 Runtime SDK 选项和即时校验状态正确。
- 审计链连续写入、验证和篡改拒绝。
- 真实 CLI 完成密钥库、采集、签发、读取、验证、篡改和批量闭环。
- 发布 ZIP 的 SHA-256、敏感文件扫描和解压后 GUI 启动通过。
- Runtime SDK ZIP 解压后可独立构建并运行最小程序。
...
- 签发员使用非管理员日常账号。
- 密钥库仅在签发期间解锁。
- 解锁后显示自动锁定倒计时，最后 60 秒告警；离开工位前主动锁定。
- 批次输出通过受控移动介质传递。
- 定期验证两份密钥库备份可恢复。
- 定期验证审计日志哈希链并将只读副本纳入受控归档。
- 签发前选择目标 Runtime SDK 系列并通过兼容性预检。
...
- 首次使用向导、中文友好配置名和统一高 DPI 界面。
- 密钥库状态门控、自动锁定倒计时、即时校验和签发确认。
- 加密备份复制/恢复核对、哈希链审计和 Runtime 兼容预检。
- 核心、UI、真实 CLI、打包、敏感文件、GUI 和 SDK 的全流程自动化测试。

完成标准：`tests/full_flow.ps1 -Version 1.0.1` 全部通过，CI 复现同一流程，Release 资产校验值可核验。
